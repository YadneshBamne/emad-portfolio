...
export function Component() {
  ...
  // Original CSS injected since the file is missing
  const STYLES = `
    .site-header-wrapper { position: fixed; top: 0; left: 0; width: 100%; z-index: 100; pointer-events: none; }
    ...
    .menu-button-text { position: relative; height: 1.2em; overflow: hidden; display: flex; flex-direction: column; font-weight: bold; font-family: monospace; text-transform: uppercase; letter-spacing: 0.1em; }
    .menu-button-text p { margin: 0; line-height: 1.2em; }
    .icon-wrap { width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; }
    .menu-button-icon { width: 100%; height: 100%; }

    .fullscreen-menu-container { position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: 90; pointer-events: none; }
    .nav-overlay-wrapper { position: absolute; inset: 0; display: none; pointer-events: auto; }
    .overlay { position: absolute; inset: 0; background: rgba(0,0,0,0.6); backdrop-filter: blur(5px); opacity: 0; }
    .menu-content { position: absolute; top: 0; right: 0; width: 100%; height: 100%; max-w-md: 400px; display: flex; flex-direction: column; justify-content: center; overflow: hidden; }
    @media(min-width: 768px) { .menu-content { width: 450px; } }
    
    .menu-bg { position: absolute; inset: 0; z-index: -1; }
    .backdrop-layer { position: absolute; inset: 0; background: #050505; transform: translateX(101%); border-left: 1px solid rgba(255,255,255,0.1); }
    .backdrop-layer.first { background: #1a1a1a; }
    .backdrop-layer.second { background: #111111; }
    
    .ambient-background-shapes { position: absolute; inset: 0; overflow: hidden; opacity: 0.6; }
    .bg-shape { position: absolute; inset: 0; width: 100%; height: 100%; opacity: 0; transition: opacity 0.5s ease; }
    .bg-shape.active { opacity: 1; }
    
    .menu-content-wrapper { position: relative; z-index: 10; padding: 3rem; }
    .menu-list { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 1.5rem; }
    .nav-link { 
    ...
    .nav-link-text { margin: 0; transform-origin: left; }
  `;
