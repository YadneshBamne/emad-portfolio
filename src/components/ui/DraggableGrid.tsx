...
export function __OriginkitBase_DraggableGrid(props: DraggableGridProps) {
    ...
    // Pin the grid initially (centered so the user has immediate room to drag in all directions)
    useEffect(() => {
        ...
        <div ref={containerRef} style={wrapperStyle} className={className}>
            ...
            >
                {displayItems.map((item, index) => {
                    ...
                    const alt = item?.alt ?? item?.image?.alt ?? item?.title ?? item?.code ?? ""
                    ...
                    return (
                        ...
                        >
                            <span style={{ position: "relative", zIndex: 0, opacity: 0.4 }}>
                                {index + 1}
                            </span>
                            {src && !failed ? (
                                <img
