...
const createClothMaterial = () => {
    ...
    `,
      ...
      void main() {
        ...
        float borderBottom = 0.2; // Thicker bottom for polaroid
        
        bool isBorder = vUv.x < borderX || vUv.x > 1.0 - borderX || vUv.y > 1.0 - borderTop || vUv.y < borderBottom;
        ...
        if (isBorder) {
          ...
          // Map UVs to the image area
          vec2 imgUv = vec2(
            (vUv.x - borderX) / (1.0 - 2.0 * borderX),
            (vUv.y - borderBottom) / (1.0 - borderTop - borderBottom)
          );
          
          color = texture2D(map, imgUv);
          ...
          if (blurAmount > 0.0) {
            ...
            for (float x = -2.0; x <= 2.0; x += 1.0) {
              for (float y = -2.0; y <= 2.0; y += 1.0) {
                ...
                blurred += texture2D(map, imgUv + offset) * weight;
                total += weight;
              }
            }
            color = blurred / total;
	...
	return (
		<>
			{planesData.current.map((plane, i) => {
				...
				// Borders used in shader
				const borderX = 0.05;
				const borderTop = 0.05;
				const borderBottom = 0.2;
				...
				const scaleX = imgWidth / (1.0 - 2.0 * borderX);
				const scaleY = imgHeight / (1.0 - borderTop - borderBottom);

				const scale: [number, number, number] = [scaleX, scaleY, 1];
